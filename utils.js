...
function initUserAuth() {
  ...
  authBtn.href = "#"; // نمنع الانتقال لصفحة التسجيل
  authBtn.classList.add("auth-chip");
// تفعيل نافذة تسجيل الخروج
  injectLogoutModal();
...
function injectLogoutModal() {
  ...
  document.getElementById("utilsConfirmLogout").addEventListener("click", () => {
    // 1. مسح بيانات المستخدم من المتصفح عشان يتم تسجيل الخروج فعلياً
    localStorage.removeItem("currentUser");
    
    // سيتم إضافة كود تسجيل الخروج الخاص بفايربيس هنا قريباً
    
    // 2. التوجيه لصفحة الدخول
